    ...
    <>
      ...
      <style>{`
        @keyframes smoothFloat {
          ...
          50% { transform: translateY(-6px); } /* Halka sa float taki clickable rahe */
      ...
      <div className="flex flex-col gap-6 px-5 pb-28 pt-6 relative">
        ...
        <div 
          className="floating-element rounded-3xl bg-brand-gradient p-6 text-primary-foreground shadow-lg shadow-primary/20"
          style={{ animationDelay: '0s' }}
        >
          <div className="flex items-center justify-between">
            <h2 className="text-xl font-bold">Daily Progress</h2>
            <span className="text-2xl font-bold bg-white/20 px-3 py-1 rounded-full">{totalPercent}%</span>
          </div>
          ...
          <div className="mt-4 h-2 w-full overflow-hidden rounded-full bg-white/30">
            <div className="h-full bg-white transition-all duration-500" style={{ width: `${totalPercent}%` }} />
          </div>
        </div>
          ...
          {habits.map((habit, index) => {
            ...
            return (
              <div 
                key={habit.id} 
                className={cn(
                  "floating-element rounded-2xl border p-4 shadow-sm transition-all", 
                  isCompleted ? "bg-secondary/40 border-primary/30" : "bg-card border-border"
                )}
                style={{ animationDelay: `${(index + 1) * 0.15}s` }} // Har card ka delay thoda alag hoga
              >
                <div className="flex items-center justify-between">
                  <div>
